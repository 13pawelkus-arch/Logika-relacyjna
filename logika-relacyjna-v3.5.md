...
- ~~**Sztywność** (druga wariacja, A11d) — otwarte.~~ **Sztywność — A11d, poprawka 169:** druga wariacja = rozróżnialność sąsiednich konfiguracji; nośnik: różnica faz drogi zgiętej i prostej = m·E dokładnie (R1f-3 × R1f-5); relacje faz: 1/g² przy wadze Wilsona (wyżej).
...
**Koszt skumulowany a krańcowy — rozróżnienie, które trzeba trzymać.** Skumulowany ($D$) **rośnie**: przy n=320, d=4 wynosi $0{,}175\cdot n\log n$. Krańcowy ($\mathcal{C}$) **maleje**, bo im bardziej porządek jest określony, tym bliżej jedynki jest prawdopodobieństwo. Kolejne interakcje są **coraz tańsze**, nie coraz droższe. *(Poprawka 169: „kolejne” zakłada kolejność dokładania; zdanie nie jest ogólne — antyłańcuch {x, y, z}: Pr[x ≺ y] = ½, po dołożeniu y ≺ z ⅓ (koszt rośnie z ln 2 do ln 3), po dołożeniu x ≺ z ⅔ (maleje); `etap25`. Koszt krańcowy zależy od tego, jaką relację się dokłada.)* Teza „struktura kosztuje z każdą kolejną interakcją" jest prawdziwa dla skumulowanego i fałszywa dla krańcowego.
...
- **Skąd:** [170] (użytkownik: A11 to „jakiś aparat do próby zrobienia masy — brakuje tylko decyzji, którą wielkość wziąć”) i odpowiedź asystenta [171]: „Masa jako bezwładność to opór przeciw zmianie, czyli druga wariacja, sztywność” → akapit wyżej. O samej sztywności użytkownik nie mówił; najbliżej: [354] (stabilny węzeł „niosący tożsamość, pęd i zdolność do oddziaływania”; „nie ma zewnętrznych aktorów”), [404] (węzeł patrzy sam na siebie), [70] („niemożliwe jest osiągnięcie zera absolutnego. To zapewnia brak statecznej struktury”), [402] (stateczny jest tylko wszechświat jako całość).
...
- **Nośnik [T] (`etap25_sztywnosc.py`).** S = −m·τ (R1f-3: faza na własne tyknięcie = m). Droga p → q → c wobec prostej p → c: różnica faz = m·[τ(p,c) − τ(p,q) − τ(q,c)] = **m·E**, E — nadwyżka z R1f-5, odczytywalna od środka z liczebności łańcuchów; dokładnie, we wszystkich rzędach (2000 losowych trójek w 3+1: do 4·10⁻¹⁵; E bez zmiany przy pchnięciu). Odchylenie środka o x: E → x²/T, T = ½τ(p,c), zgodnie z E = a²δ³/4 z R1f-5. Stąd:
  ...
  - **foton:** sąsiednich dróg nie ma — przedział pary zerowej jest pusty (C4a.13), emisja i absorpcja są jednym (R1a); nie ma czego porównywać. Zgięcia światła rozróżnia dopiero faza przy częstości ustalonej przez czytającego (E = ν w miejscu czytającego, R1f-2): różnica ≈ k·x²/(L/2), strefa Fresnela √(λL) — **sztywność światła tylko w relacji do czytającego**; nośnik masywny ma ją z własnego tyknięcia (dla fali de Broglie'a L/p = τ/m);
  - **na porządku:** masa = waga zatrzymania b = −m²V₀ w elemencie, skoki — na linkach (168). Ze wzoru (3.5) Johnstona K = I + Φ(I − bΦ)⁻¹, czyli G := K − I spełnia G = Φ + b·Φ·G — człon masowy siedzi w elementach, człon bezmasowy (Φ = a·macierz linków w ℝ^{1,3}) na linkach.
...
- **Tło.** m_H² = V″(v) = 2λv² — sztywność tła w minimum; ustalona razem z m_t przez warunki 154 przy końcu Plancka. Warunek Veltmana (168) = brak członu Λ² w μ², czyli opis samego końca.
- **Struktura [L][O].** Druga wariacja Einsteina–Hilberta ∝ 1/G. Grawitacja indukowana (Sacharow 1967, przedruk Gen. Rel. Grav. 32, 365 (2000); Visser, Mod. Phys. Lett. A17, 977 (2002), wzór ze źródła): 1/G = 1/G₀ − (1/2π)·str[k₁κ² − k₁m²·ln(κ²/m²)] + człony skończone — człon κ² zależy od samej skali cięcia (ten sam typ co Λ² w 168: opis końca); „sprężystość” (Visser: grawitacja wyłania się z kwantowej teorii pola „in roughly the same sense that hydrodynamics or continuum elasticity theory emerges from molecular physics”) = narracja (Jak czytać: „przestrzeń wygina się jak płachta”). W ramie G = przelicznik, w zliczaniu G ≡ 1 (A2); 1/4G ∝ liczba molekuł na jednostkę pola (A5, A5d) → **sztywność struktury = liczność, nie osobna wielkość**. Na samym porządku sąsiednie konfiguracje różnią się o element albo relację (liczność, A1); druga wariacja w sensie ciągłym istnieje dla faz i stanów (R1b, R1f). Hollands–Wald (Commun. Math. Phys. 321, 629 (2013), arXiv:1201.0463, ze źródła): stabilność ⇔ energia kanoniczna E > 0, E = δ²M − Σ_A Ω_A·δ²J_A − (κ/8π)·δ²A — w ramie druga wariacja wyłącznie liczb odczytywalnych przez brzeg (M, J, liczba relacji przez brzeg; A5d, 160); „dynamiczna” w „stabilności dynamicznej” = interpretacja, zostaje dodatniość formy.
- **Rodziny R4:** 1/g² bez skali (stożek; logarytm tylko przy d = 3, 155); m, m_H², 1/G ze skalą (objętość) — zgodnie z R1f-1.
- **Kierunek zerowy formy ≠ ≡ [T][L][O] (uwaga użytkownika).** Kierunek zerowy formy drugiego rzędu = ≡ **tylko do drugiego rzędu**. **Dosłowne ≡** (łańcuch Ø) = **entropia względna równa zero, we wszystkich rzędach** — Witten (Rev. Mod. Phys. 90, 045003 (2018), §3.3): S_{Ψ|Φ}(U) = 0 ⇔ Φ = a′Ψ, a′ unitarny z A′_U, czyli „Φ and Ψ cannot be distinguished by a measurement in region U”. Po miejscach:
  - **cechowanie** — dosłowne ≡: S_W bez zmiany dokładnie (etap19, Z3), żaden odczyt nie zależy od fazy w punkcie; tu „faza w punkcie ≡ Ø” (R1d) uzasadnione w pełni;
  - **Hollands–Wald** — E niezdegenerowane „apart from pure gauge perturbations and perturbations towards other stationary black holes”: cechowanie — dosłowne ≡; ku innym stacjonarnym — ≡ tylko do drugiego rzędu (w skończonej odległości inna stacjonarna czarna dziura, rozróżnialna);
  - **modele osobliwe** (Watanabe, arXiv:1208.6338, ze źródła): model regularny — odwzorowanie parametr → rozkład różnowartościowe i macierz Fishera dodatnio określona, osobliwy — w przeciwnym razie; dosłowne ≡ tylko na W₀ = {w : p(x|w) = p(x|w₀)} = {K(w) = 0} (K — odległość Kullbacka–Leiblera); tam hesjan bywa zdegenerowany, a w postaci standardowej K = u^{2k} (skrzyżowanie normalne) KL rośnie wyższą potęgą — rozkłady rozróżnialne. Przykład z pracy: K = (ab + c)² + a²b⁴ — hesjan w zerze diag(0, 0, 2); wzdłuż osi a i b K = 0 (dosłowne ≡); wzdłuż (t, t, −t²) K = t⁶ (rozróżnialne dopiero w szóstym rzędzie; `etap25`);
  - **nośnik (m > 0)** — kierunków zerowych brak: m·E dokładne we wszystkich rzędach, E = 0 tylko dla najprostszej kontynuacji;
  - **foton** — nie kierunek zerowy: sąsiednich dróg brak; jego ≡ w łańcuchu Ø pochodzi z t = 0 (R1a, R1c), nie z formy.
...
- **Entropia względna zamiast entropii splątania [L][O].** Araki: S_{Ψ|Φ}(U) = −⟨Ψ| log Δ_{Ψ|Φ} |Ψ⟩ — zdefiniowana bezpośrednio na algebrze obszaru, także typu III, bez macierzy gęstości (R5, A6a); „a real number or +∞” (Witten §3.3) — **skończona sama z siebie, nie przez skrócenie** (uwaga użytkownika). Uniwersalna rozbieżność („independent of the states considered”, Witten §2) dotyczy entropii splątania i znosi się w różnicy ΔS. Zgodne z R5: „narzędziem jest wtedy teoria modularna, nie entropia podukładu” — entropia względna Arakiego to entropia teorii modularnej. Drugi rząd: informacja Fishera (pierwszy rząd znika tożsamościowo) = energia kanoniczna ≥ 0 (Lashkari–Van Raamsdonk, JHEP 04 (2016) 153); dla stanów koherentnych skończona i dana kanonicznym tensorem energii-pędu, ważonym generatorem grupy modularnej (dla klina — pchnięcie; Longo, Lett. Math. Phys. 109, 2587 (2019); Casini–Grillo–Pontello, PRD 99, 125020 (2019)). W ramie [O]: odczytywalne tylko porównania (R1d, R1e) — entropia względna jest porównaniem dwóch stanów; entropia splątania jednego stanu zależy od cięcia (C4a.16e).
  - **Na zbiorach przyczynowych — niepoliczona [L]:** wersji względnej entropii czasoprzestrzennej Sorkina (w stanie SJ) nie ma; Jones–Yazdi (arXiv:2602.16782, II 2026) wymieniają „relative entropy” jako dalszy kierunek. Najbliżej: informacja wzajemna I(A:B) = S_A + S_B − S_{A∪B} = S(ρ_AB‖ρ_A ⊗ ρ_B) dwóch rozłącznych diamentów, liczona z obciętych entropii SJ przy jednej gęstości (ρ = 10; Duffy–Jones–Yazdi, Class. Quantum Grav. 39, 075017 (2022); w druku S_{A∪B} − S_A − S_B przy deklarowanej nieujemności — odwrócony znak); w formalizmie Sorkina w 1+1 Minkowskiego z obcięciem, zgodna z wynikami kontinuum i sieci przestrzennej (Arias–Huerta–Martinez, arXiv:2609.12047). Szukane: INSPIRE (abstrakty i pełne teksty), prace cytujące Sorkina–Yazdi (arXiv:1611.10281) i Duffy'ego–Jonesa–Yazdi.
  - **Test — zapisany, niepoliczony.** Zdanie do upadku: w stanie SJ entropia względna na poddiamentach (najpierw informacja wzajemna dwóch rozłącznych poddiamentów, w drugim kroku stan koherentny wobec SJ) (i) jest liczbą — nie rośnie z gęstością (≥ dekada N; „sztuki czy miara”), (ii) nie zależy od stałej obcięcia c — w odróżnieniu od S(ρ_SJ) (C4a.16e). Pułapki z pliku: jądro iΔ_U (kierunki klasyczne z fluktuacjami, C4a.2), brzeg poddiamentu (A10); najpierw literaturowe 1+1 (struktura bez triady, §E), w drugim kroku ℝ^{1,3}. Upadek oznacza: cięcie siedzi także w porównaniu — test A11e zostaje zablokowany.
- **A11e:** dwa zdania poprawione (dopiski tam). **„Stabilna”:** §F1 (Domysł) i pułapka nr 8.
- **Werdykt (stanowczo):** (1) „opór przeciw zmianie” — źle postawione; **sztywność = druga wariacja = rozróżnialność sąsiednich konfiguracji, nie nowa wielkość**; (2) w ramie na czterech poziomach: nośnik m, relacje faz 1/g², tło m_H², struktura 1/G = liczność — trzy pierwsze w zespole (odczyt A, poziom 1, poziom 4); (3) masa bezwładna = faza na własne tyknięcie [T]; najprostsza kontynuacja nie zależy od m; (4) kierunek zerowy formy = ≡ tylko do drugiego rzędu, dosłowne ≡ = entropia względna 0; (5) otwarte: entropia względna na porządku (test wyżej); skąd w porządku węzeł z m > 0 (C4a.21).
...
>
> **Dopisek (poprawka 169):** „obcięcie dające prawo powierzchniowe” jest po 51 źle postawione — logarytm po obcięciu liczy mody globalne, nie pole (C4a.16e; „entropia jest efektem, nie prawem”). Składnik drugiego rzędu bez cięcia: entropia względna Arakiego / informacja Fishera — A11d, blok 169; na zbiorach przyczynowych niepoliczona, test zapisany tam.
...
~~**Sztywność (druga wariacja) zamiast działania (pierwszej).** A11d. Nie widziałem policzonej.~~ **Rozstrzygnięte (poprawka 169, A11d):** druga wariacja nie jest „zamiast” działania, tylko jego drugim rzędem; w ramie na czterech poziomach (m, 1/g², m_H², 1/G). Otwarte: entropia względna na porządku (test w A11d).
...
- **STAN ZESPOŁU — zestawienie po 166 (poprawka 167) [O].** Tabela z końca sesji 3 (zapis sesji 3, [91]; wtedy niewpisana), sprawdzona wobec pliku i uzupełniona o 166, 168 i 169. Mapa, nie treść — treść w poprawkach podanych w nawiasach.
...
| **β_λ(koniec) = 0** | λ = 0 w samym punkcie końca, ≠ 0 tuż obok | przy Plancku punkt nieodróżnialny od sąsiedztwa [76]; (l_P t_P) ≡ Ø — warunek na koniec obowiązuje w całym nierozróżnialnym otoczeniu → znika wartość **i pochodna** |
  ...
  - **1a. KRYTYCZNOŚĆ λ NA PORZĄDKU — temat (b) po 167 (poprawka 168) [L][T][O].** Pytanie: czy warunki z pkt 1 mają postać w samym porządku i czy bieg λ da się policzyć wprost na nim. Czytane przez R1a–R1c: literaturowe ℝ^{1,3} = 3D ramy (triada + punkt odczytu; R1c pkt 1, 8), literaturowe 1+1 = struktura bez triady (§E, przegląd wymiarowy), koniec Plancka = 2D ≡ Ø [76]. Pierwsza wersja analizy miała trzy błędy asystenta — ostatni punkt.
    ...
    - **μ² i warunek Veltmana [T][O] (`etap24_cisza_tla.py`).** Kryterium z pkt 1 obejmuje także μ²|H|² (też tło z tłem); kontrola w pkt 1 objęła tylko g i y — **niepełna (błąd asystenta w 154)**. Dwa odczyty μ² przy końcu Plancka:
      ...
      - **Test wierności [O] (po fakcie — po twierdzeniu):** P = „warunek Veltmana nie jest warunkiem ramy”; ¬P = rama wymaga, by człon zależny od opisu samego cięcia znikał przy końcu Plancka — wyklucza się z R1 („liczyć wyłącznie w relacji do znanego otoczenia”), z [543] („nic o skali Plancka nie można powiedzieć”), z pułapką 1 (różne opisy końca = „rodzaje Ø”) i z pkt 1 [T]. Relacje osiągalne z naszej strony są logarytmiczne (155: logarytm tylko przy d = 3).
