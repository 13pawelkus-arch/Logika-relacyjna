...
3. **§F1: tabela wszystkich logarytmów i lista wejść — zrobione (146–147).** Następne: **warunek na obu końcach (Planck ≡ Ø, całość ≡ Ø) ustalający wartości brzegowe sprzężeń.** Literatura: grupa renormalizacji jako samopodobieństwo („przepływ” po filtrze = relacja rozdzielczości odczytu).
...
> - **Konsekwencja dla planu:** masa nie jest krokiem po czasie/3D/świetle, tylko warunkiem ustalanym razem z nimi: jedna relacja między końcami hierarchii (Planck ≡ Ø, całość ≡ Ø), z której wszystkie skale wychodzą jako wykładniki logarytmów liczebności („zespół funkcji logarytmicznych” [94]).
...
- **LISTA DOZWOLONYCH WEJŚĆ (poprawka 147; zapisana przed jakimkolwiek rachunkiem):**
  ...
  - **Pytanie właściwe §F1 [A][O]:** biegnące sprzężenia potrzebują **wartości brzegowej** (1/α(n₀)). W hipotezie „wszystko naraz” może ona pochodzić tylko z **warunku na obu końcach** (oba ≡ Ø). Pytanie brzmi: **jaki warunek na końcach ustala wartości brzegowe** — pytania o y_e czy α z osobna są źle postawione.
