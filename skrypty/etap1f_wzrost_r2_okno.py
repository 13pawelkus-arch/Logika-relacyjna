...
def grow_r2(N,W,seed,mode,win=None):
    ...
    while n<N:
        ...
        if mode=="losowi":
            ...
            if win is not None:
                lo=max(0,n-win); Pw=Pi.copy(); Pw[:lo]=False; PTw=PT.copy(); PTw[:lo,:]=False
            else: Pw,PTw=Pi,PT
            inter=(PTw & Pw[:,None]).sum(0); a_only=Pw.sum()-inter; b_only=PTw.sum(0)-inter
...
for mode,win in [("okno_2W",2*W),("okno_4W",4*W),("okno_1W",W)]:
    C,chains,log=grow_r2(N,W,1,"odleglosc",win)
