...
- **Nie ma kierunku czasu jako pierwotnego:** odczyt jest zawsze teraz, przeszłość to zapis czytany teraz, strzałka to pseudokierunek z rozproszenia. Nie przemycać „wcześniej–później” z kolejności budowania symulacji (poprawka 106).
...
1. **Definicja czasu** — największa zmiana (pełna synteza w R1a, 25.09): czas = odczyt informacji ze struktury, zawsze teraz; odczyt tworzy czwarty punkt odniesienia (R1a, „Warunki muszą zachodzić razem”). **Prędkość c** = tempo przekazu informacji, nie pokonywania dystansu (C5; metr od 1983, Reichenbach). Bez porządnej definicji czasu nikt nie ruszy z miejsca.
...
- **Literatura krzywizny (25.09):** krzywizna Olliviera zbiega tylko mezoskopowo, więc nasze pomiary na skali ogniwa nie mogły zejść do zera. Istnieje krzywizna dla zbiorów przyczynowych wzdłuż łańcuchów maksymalnych (trajektorii) i horyzonty przez ogniskowanie. Pytania P-K1–P-K3 w C5.
- **Triada + zapis (25.09, rysunek w `rysunki/`):** odczyt = czworościan, odczyty = fotony = linki; Regge na kompleksie: średnio przechodzi przez płaskie (5,104), lokalnie 4 albo 8, średnia dryfuje jak ln W (etap18). Dynamika (falujące boki) niemierzona.
